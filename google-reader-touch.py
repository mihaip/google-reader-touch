...
TOUCH_TAG_PREFIX = 'user/-/state/com.google/touch'
...
def TouchTags():
  ...
  for tag in TAGS:
    ...
    for i in xrange(0, len(item_ids), EDIT_TAG_CHUNK_SIZE):
      ...
      try:
        # Cycle through touch tags, so that no individual one ends up with too
        # many items.
        touch_tag = '%s-%s' % (TOUCH_TAG_PREFIX, str(int(time.time())/(60 * 5)))
...
def CleanupTags():
  print 'Cleaning up touch tags'
  tag_list_json = json.loads(ExecuteMethod('tag/list?output=json'))
  for tag_json in tag_list_json['tags']:
    tag_stream_id = tag_json['id']
    if tag_stream_id.startswith(TOUCH_TAG_PREFIX):
      print 'Deleting %s...' % tag_stream_id
      ExecuteMethod('disable-tag', [('s', tag_stream_id)])
      print '  deleted'

parser = optparse.OptionParser()
parser.add_option(
    '--mode', dest='mode', default='touch',
    help='"touch" (add touch tags) or "cleanup" (remove touch tags)')
